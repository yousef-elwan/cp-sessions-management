...
async def create_role_permission(role_id: int, permission_id: int, db: AsyncSession = Depends(get_db)):
...
async def toggle_role_permission(rp_id: int, db: AsyncSession = Depends(get_db)):
